...
:match_regex
	...
	REM empty result var
	for /l %%K in (0, 1, 10) do (
		set "v=!_var_prefix_match_regex!_%%K"
		set "!v!="
	)
...
:uri_parse
	...
	if not "!_query!"=="" (
		...
		set "_i=1"
		:loop_uri_parse
			set "_arg=_tmp_prefix_query_!_i!"
			for /F %%V in ('echo !_arg!') do set "_arg=!%%V!"

			set /a "_i+=2"
			set "_val=_tmp_prefix_query_!_i!"
			for /F %%V in ('echo !_val!') do set "_val=!%%V!"
			...
			if not "!_arg!"=="" goto :loop_uri_parse
